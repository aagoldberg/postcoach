...
export default function Home() {
  ...
  return (
    <div className={`relative min-h-screen transition-colors duration-500 ${theme === 'cyberpunk' ? 'theme-cyberpunk bg-[#272822] text-[#66D9EF]' : 'bg-[#f2f5f3] text-[#1a1f2e]'} font-sans`}>
      ...
      <main className="container mx-auto px-4 py-8 md:py-32 max-w-7xl relative z-10">
          ...
          <div className="flex flex-col items-center justify-center py-12 md:py-20 animate-in fade-in slide-in-from-bottom-12 duration-1000">
            <div className="text-center space-y-8 md:space-y-16 max-w-4xl mx-auto w-full">
              <div className="space-y-6 md:space-y-8">
                <div className={`inline-flex items-center gap-3 px-5 py-2 rounded-full border shadow-sm mb-4 ${theme === 'cyberpunk' ? 'bg-[#272822] border-[#66D9EF] text-[#66D9EF]' : 'bg-white border-stone-200 text-stone-400'}`}>
                  <span className={`w-2 h-2 rounded-full animate-pulse ${theme === 'cyberpunk' ? 'bg-[#F92672]' : 'bg-[#4b5e54]'}`}></span>
                ...
                <p className={`text-lg md:text-2xl font-medium leading-relaxed max-w-xl mx-auto px-4 ${theme === 'cyberpunk' ? 'text-[#66D9EF] font-mono tracking-widest drop-shadow-[0_0_5px_rgba(102,217,239,0.8)]' : 'text-stone-400'}`}>
              ...
              <div className={`pt-16 md:pt-32 flex flex-col md:flex-row items-center justify-center gap-8 md:gap-16 text-[10px] font-bold uppercase tracking-[0.4em] ${theme === 'cyberpunk' ? 'text-[#F92672]' : 'text-stone-300'}`}>
                <div className="flex flex-col items-center gap-4">
                  <div className={`w-12 h-12 rounded-[1.5rem] border shadow-sm flex items-center justify-center ${theme === 'cyberpunk' ? 'bg-[#272822] border-[#F92672] text-[#F92672] rounded-none' : 'bg-white border-stone-100 text-[#1a1f2e]'}`}>
        ...
        {state === 'error' && (
          <div className="flex-1 flex flex-col items-center justify-center py-64">
             <div className={`max-w-xl text-center p-20 rounded-[3.5rem] shadow-2xl ${theme === 'cyberpunk' ? 'bg-[#272822] border border-[#F92672] rounded-none shadow-[0_0_30px_rgba(249,38,114,0.3)]' : 'bg-white'}`}>
              <div className={`w-24 h-24 rounded-[2rem] flex items-center justify-center mx-auto mb-10 ${theme === 'cyberpunk' ? 'bg-[#F92672]/20 text-[#F92672] rounded-none border border-[#F92672]' : 'bg-rose-50 text-rose-500'}`}>
                <svg className="w-10 h-10" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
                </svg>
              </div>
              <h2 className={`text-4xl font-black mb-4 serif-heading italic ${theme === 'cyberpunk' ? 'text-[#F8F8F2] font-mono not-italic' : 'text-[#1a1f2e]'}`}>Audit Interrupted</h2>
              <p className={`mb-12 text-lg font-medium leading-relaxed ${theme === 'cyberpunk' ? 'text-[#66D9EF]' : 'text-stone-400'}`}>{error}</p>
              <Button
                onClick={handleReset}
                variant="primary"
                size="lg"
                className={`w-full py-6 ${theme === 'cyberpunk' ? 'rounded-none bg-[#66D9EF] text-[#272822] hover:bg-[#F8F8F2]' : 'rounded-3xl'}`}
              >
                Return to Lab
              </Button>
            </div>
